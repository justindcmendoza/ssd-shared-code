...
import { filterItems, filterNonVesselDelivery, filterCareOffs } from "../helpers/JobHelpers";

const validateJobCreation = async (data, vesselLoadingLocations, apiUri, edit) => {
    const job = { ...data };
    ...
    if (
        ...
        try {
            const res = await axios.get(`${apiUri}/api/vessels/search?query=${queryVesselName}`);
    ...
    if (filteredItems.length < 1 && filteredJobOfflandItems.length < 1) {
        return {
            valid: false,
            message: "At least one item must be submitted for delivery or offlanding!",
        };
    }
    ...
    if (!job.vesselLoadingLocation.type) {
        return {
            valid: false,
            message: "Vessel loading location is required",
        };
    }

    // Check Other vessel loading location
    if (job.vesselLoadingLocation.type === "others" && job.otherVesselLoadingLocation.name.trim() === "") {
        return {
            valid: false,
    ...
    if (job.makeLighterBooking && job.vesselLoadingLocation.type !== "anchorage") {
        return {
            valid: false,
            message: "Invalid vessel loading location for selected booking type.",
        };
    }
    ...
    if (
        ...
        (job.offlandDetails.length === 0 ||
            (job.offlandDetails.length === 1 && job.offlandDetails[0].offlandLocation.addressString.trim() === ""))
    ) {
        return {
            valid: false,
    ...
    if (job.pickupDetails.length >= 0) {
        ...
        for (let i = 0; i < pickupDetails.length - 1; i++) {
            ...
            if (moment(pickupDetailB.pickupDateTime).isBefore(moment(pickupDetailA.pickupDateTime).add(5, "minutes"))) {
                return {
                    valid: false,
                    message: "Pickup Date & Time must be at least 5 minutes apart",
                };
            }
        }
        for (let i = 0; i < pickupDetails.length; i++) {
            ...
            if (
                ...
                return {
                    valid: false,
                    message: "Pickup Date & Time must be after Job creation time",
                };
            }
            if (
                job.vesselLoadingLocation.type !== "port" &&
                moment(pickupDetail.pickupDateTime).isAfter(moment(job.vesselLoadingDateTime))
            ) {
                return {
                    valid: false,
                    message: "Pickup Date & Time must be before Job delivery time",
                };
            }
        }
    }
    ...
    if (job.offlandDetails.length >= 0) {
        ...
        for (let i = 0; i < offlandDetails.length - 1; i++) {
            ...
            if (
                ...
                return {
                    valid: false,
                    message: "Offland Date & Time must be at least 5 minutes apart",
                };
            }
        }
        for (let i = 0; i < offlandDetails.length; i++) {
            ...
            if (
                ...
                return {
                    valid: false,
                    message: "Offland Date & Time must be after Job creation time",
                };
            }
        }
    }
    ...
    if (!edit && job.vesselLoadingLocation.type !== "port" && moment(job.vesselLoadingDateTime).isBefore(new Date())) {
        return {
            valid: false,
            message: "Vessel Loading Date & Time must be after job booking time",
        };
    }

    if (job.jobPICName.trim() !== "" && job.jobPICContact.toString().trim() === "") {
        return {
            valid: false,
            message: "Please enter a valid PIC contact",
        };
    } else if (job.jobPICContact.toString().trim() !== "" && job.jobPICName.trim() === "") {
        return {
            valid: false,
    ...
    if (job.hasBoarding && job.boardingName.trim() !== "") {
        ...
        if (!boardingName.match(letters)) {
            return {
                valid: false,
                message: "Please enter a valid boarding name, with only alphabets.",
            };
        }
    }
    ...
    if (job.hasBoarding && job.boardingContact.toString().trim() !== "") {
        ...
        if (!boardingContact.match(numbers)) {
            return {
                valid: false,
                message: "Please enter a valid boarding contact, with only numbers.",
            };
        }
    }

    if (job.vesselLoadingLocation.type === "anchorage" && (!job.lighterBoatCompany || !job.lighterBoatCompany._id)) {
        return {
            valid: false,
            message: "Please select a Lighter Company",
        };
    }
